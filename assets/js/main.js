...
const keywords = [
	...
	"Affordable E-commerce Web Development in Hyderabad",
	"Top Digital Marketing Agencies in Hyderabad",
	"Best Custom Web Design Services in Hyderabad",
	"Best Web Designing Company in Hyderabad",
	"Freelance Website Designer in Hyderabad",
	"Low-Cost Web Design in Hyderabad",
	"Low-Cost Website Design in Hyderabad",
	...
	"Website Builders in Hyderabad",
	"Professional SEO Services for Businesses in Hyderabad",
	"Web Design Near Me",
	"Website Designers in Hyderabad",
	"Web Design Company in Hyderabad, India",
...
function generateKeywordLines() {
	container.innerHTML = ""; // Clear previous lines

	let keywordsPerLine = 5; // Default for desktop
	const width = window.innerWidth;

	if (width <= 1024 && width > 768) keywordsPerLine = 4;
	else if (width <= 768 && width > 600) keywordsPerLine = 3;
	else if (width <= 600) keywordsPerLine = 2;

	for (let i = 0; i < keywords.length; i += keywordsPerLine) {
		const line = document.createElement("div");
		line.classList.add("keyword-line");

		const group = keywords.slice(i, i + keywordsPerLine);
...
// const container1 = document.getElementById("techContainer");
// const tabs = document.querySelectorAll("#techTabs .nav-link");

// function renderTech(category) {
// 	container1.innerHTML = techData[category]
// 		.map(tech => `
